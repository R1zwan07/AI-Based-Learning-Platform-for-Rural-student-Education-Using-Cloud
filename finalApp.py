...
app.config['UPLOAD_FOLDER'] = os.path.join(app.root_path, 'pdf')
...
def upload_roadmap():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            db_manager.save_roadmap(std_class, filename)
    ...
    return render_template('upload_roadmap.html', roadmaps=roadmaps)
...
def save_roadmap(std_class, pdf_name):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO roadmaps (std_class, pdf_name) VALUES (%s, %s)",
        (std_class, pdf_name)
    )
    conn.commit()
    cursor.close()
...
def delete_roadmap(roadmap_id):
    try:
        # Delete the roadmap record from the database
        db_manager.delete_roadmap(roadmap_id)
        flash("✅ Roadmap deleted successfully!", "success")
    except Exception as e:
        flash(f"❌ Error deleting roadmap: {e}", "error")
    return redirect(url_for('upload_roadmap'))

@app.route('/view_roadmap')
def view_roadmap():
    # Fetch all roadmaps from the DB
    conn = db_manager.get_db_connection()
    ...
    return render_template('viewroadmap.html', roadmaps=roadmaps)

@app.route('/pdfs/<filename>')
def serve_roadmap_pdf(filename):
...
def update_material(material_id):
    ...
    update_pdf_files = request.files.getlist('update_pdf_file')
    ...
    updated_pdf_path = material['pdf_path'] or ''
    ...
    # ✅ Update PDFs — add new ones while keeping existing
    pdf_paths = updated_pdf_path.split(',') if updated_pdf_path else []

    for pdf in update_pdf_files:
        if pdf and pdf.filename != '':
            pdf_path = save_uploaded_file(pdf, UPLOAD_DIR)
